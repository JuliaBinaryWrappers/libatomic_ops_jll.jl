...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import libatomic_ops_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("libatomic_ops_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "libatomic_ops_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"libatomic_ops", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "libatomic_ops_jll", "src", "libatomic_ops_jll.jl"))
        @info("libatomic_ops_ll dev'ed out to /depot/dev/libatomic_ops_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("05a86a2e-8047-56b1-a9eb-0973326f4c2d"))
platforms = [Pkg.Artifacts.unpack_platform(e, "libatomic_ops", artifacts_toml) for e in artifacts["libatomic_ops"]]
...
if best_platform === nothing
    @debug("Unable to load libatomic_ops; unsupported platform $(triplet(platform_key_abi()))")
else
